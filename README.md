...
- A correct answer scores 500 points plus up to 500 more depending on how much
  time was left. Wrong or missing answers score nothing. The player page shows
  what a right answer is worth, counting down with the clock; once you pick,
  it shows what your pick earns if it's right, and the reveal splits the
  points into the 500 and the speed bonus.
...
- `GET /qr.svg` — the join QR code as its own image, black on white, so
  extensions that recolour pages (Dark Reader) can't make it unscannable.
...
- `POST /api/answer` — the answer form (`seq`, `choice`), posted by htmx
  whenever the picked radio button changes; `204` on success, with an
